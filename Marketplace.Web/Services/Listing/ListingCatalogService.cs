...
{
    private static readonly List<ListingItemViewModel> Seed = GenerateSeed().ToList();

    private static IReadOnlyList<ListingItemViewModel> GenerateSeed()
    {
        var categories = new[]
        {
            ("beauty", "Beauty"),
        ...
        var list = new List<ListingItemViewModel>();
        int id = 1;
        ...
        for (int i = 0; i < 40; i++)
        {
            ...
            list.Add(new ListingItemViewModel
            {
                Id = id++,
                Title = title,
    ...
    public Task<IReadOnlyList<ListingCategoryViewModel>> GetCategoriesAsync(
        CancellationToken cancellationToken = default)
    {
        IReadOnlyList<ListingCategoryViewModel> categories =
        [
    ...
    public Task<PagedResult<ListingItemViewModel>> GetListingsAsync(
        ListingsFilterRequest request,
        CancellationToken cancellationToken = default)
    {
        IQueryable<ListingItemViewModel> query = Seed.AsQueryable();
        ...
        return Task.FromResult(new PagedResult<ListingItemViewModel>
        {
            Items = items,
            Page = page,
            PageSize = pageSize,
    ...
    public Task<ListingItemViewModel?> GetByIdAsync(int id, CancellationToken cancellationToken = default)
    {
        var item = Seed.FirstOrDefault(x => x.Id == id);
        return Task.FromResult(item);
    }

    public Task CreateAsync(ListingItemViewModel model, CancellationToken cancellationToken = default)
    {
    ...
    public Task UpdateAsync(ListingItemViewModel model, CancellationToken cancellationToken = default)
    {
