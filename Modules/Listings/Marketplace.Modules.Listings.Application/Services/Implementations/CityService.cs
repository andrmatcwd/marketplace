...
public class CityService : ICityService
{
    ...
    public CityService(ICityRepository cityRepository, ISlugService slugService, IMapper mapper)
    {
    ...
    public async Task EditAsync(EditCityCommand command, CancellationToken cancellationToken)
    {
        ...
        if (city == null) throw new Exception($"City with id {command.Id} not found.");

        city.Name = command.Name;
        city.Slug = string.IsNullOrWhiteSpace(command.Slug)
            ? _slugService.Generate(command.Name)
            : command.Slug.Trim().ToLowerInvariant();
        city.Description = command.Description;
        city.IsPublished = command.IsPublished;
        city.SortOrder = command.SortOrder;
